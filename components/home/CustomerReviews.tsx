...
const REVIEWS: Review[] = [
  {
    ...
    content:
      "다른 사이트에서는 자꾸 1G 고가 요금제랑 부가서비스를 권해서 부담스러웠거든요. 여기는 1인 가구인 제 사용 패턴에 맞춰 100M 요금제로 깔끔하게 안내해 주셔서 좋았습니다.",
  ...
  },
  {
    ...
    content:
      "알뜰폰을 쓰고 있어서 결합 할인은 기대도 안 했는데, 상담원분이 제 통신사 망에 맞춰서 제일 알뜰한 결합 상품을 안내해 주셨어요. 덕분에 매달 나가는 통신비가 확 줄었습니다!",
  ...
  },
  {
    ...
    content:
      "이사 날짜가 타이트해서 설치가 늦어질까 봐 걱정했는데, 원하는 시간대에 딱 맞게 엔지니어 기사님 방문 예약을 잡아주셨어요. 신혼부부 전용 할인 혜택까지 챙겨받았습니다!",
...
function ReviewCard({ review, className = "" }: { review: Review; className?: string }) {
  return (
    <div data-aos="fade-up" className={`card w-full rounded-2xl px-5 py-8 pc:px-8 pc:py-10 ${className}`}>
  ...
  return (
    <section>
      <div className="relative">
        ...
        {/* 모바일: w-full, flex-col */}
        <div className="mt-10 flex w-full flex-col gap-6 pc:hidden">
        ...
        <div className="hidden pc:mt-20 pc:grid pc:grid-cols-2 pc:gap-x-11 pc:gap-y-8">
          ...
          <div className="relative pc:z-30 pc:row-span-2 pc:self-center">
            <Image
              src="/images/큰따옴표2.png"
              alt="큰따옴표"
              width={44}
              height={37}
              className="absolute right-5 bottom-15 z-10"
            />
            <ReviewCard review={review2} className="pc:mb-20" />
          </div>
          <ReviewCard review={review3} className="pc:ml-30 pc:mt-2 pc:z-10" />
        </div>
      </div>
    </section>
