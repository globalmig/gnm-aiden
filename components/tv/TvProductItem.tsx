...
export default function TvProductItem({ product }: { product: Product }) {
  return (
    <div className="flex h-full flex-col rounded-2xl border border-[#ccc] p-6">
      ...
      <p className="mt-1 truncate text-xl leading-snug font-semibold text-title">{product.name}</p>
      <p className="mt-1 text-xs text-muted pc:mt-2">{specStr(product.specs, "model")}</p>
      ...
      <ul className="mt-4 flex flex-col">
        {SPEC_LABELS.map(({ key, label }) => (
          ...
          >
            ...
            <span className="text-muted">{specStr(product.specs, key)}</span>
          </li>
        ))}
      </ul>
