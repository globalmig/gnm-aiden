...
export default function Footer() {
  ...
  return (
    <footer className="mt-auto bg-sky-light">
      <div className="mx-auto max-w-300 px-5 py-10 pc:px-0 pc:py-16 [&_:is(p,li,a)]:text-sm">
        <div className="card space-y-4 p-6 pc:p-10">
          <h2 className="font-bold text-[#777]">{COMPANY_INFO.companyName}</h2>
          <div className="border-t border-gray-100" />

          <ul className="space-y-2">
            {INFO_ROWS.map((row) => (
              <li key={row.label} className="text-[#AAA]">
                <span className="font-semibold text-[#777]">{row.label}. </span>
                  {row.href ? (
                    <Link href={row.href}>
                      {row.value}
                    </Link>
          ...
          <p className="pt-2 leading-relaxed text-[#AAA]">{COMPANY_INFO.disclaimer}</p>
        </div>

        <div className="card mt-4 flex flex-col gap-2 p-5 pc:flex-row pc:items-center pc:justify-between">
          <ul className="flex flex-wrap items-center gap-x-2 gap-y-1">
            {FOOTER_LINKS.map((link, index) => (
              <li key={link.agreementId} className="flex items-center gap-x-2">
                {index > 0 && <span className="hidden pc:inline-block" aria-hidden="true">|</span>}
                <button type="button" onClick={() => setOpenModal(link.agreementId)}>
                  {link.label}
                </button>
              </li>
            ))}
            <li><span className="hidden pc:inline-block"> |</span> Copyright {COMPANY_INFO.name} {new Date().getFullYear()} All rights reserved.</li>
          </ul>
        </div>
      </div>
      ...
      <button
        ...
        aria-label="맨 위로 이동"
        className="fixed right-5 bottom-40 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-primary text-white transition-opacity hover:opacity-90"
      >
